...
# 获取项目根目录（constant.py 所在目录的父目录）
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# webui config path (使用绝对路径)
WEBUI_CONFIG = os.path.join(PROJECT_ROOT, "data", "webui_config.json")
WEBUI_CONFIG_BACKUP = os.path.join(PROJECT_ROOT, "data_backup", "webui_config.json")
...
# presets data path (使用绝对路径)
PRESETS = os.path.join(PROJECT_ROOT, "presets")
PRESETS_BACKUP = os.path.join(PROJECT_ROOT, "presets_backup")
...
# # msst model map path
# MSST_MODEL = os.path.join(PROJECT_ROOT, "data", "msst_model_map.json")
# MSST_MODEL_BACKUP = os.path.join(PROJECT_ROOT, "data_backup", "msst_model_map.json")

# vr model map path
# VR_MODEL = os.path.join(PROJECT_ROOT, "data", "vr_model_map.json")
# VR_MODEL_BACKUP = os.path.join(PROJECT_ROOT, "data_backup", "vr_model_map.json")

# language data path (使用绝对路径)
LANGUAGE = os.path.join(PROJECT_ROOT, "data", "language.json")
LANGUAGE_BACKUP = os.path.join(PROJECT_ROOT, "data_backup", "language.json")

# path to models information config (使用绝对路径)
MODELS_INFO = os.path.join(PROJECT_ROOT, "data", "models_info.json")

# path to pretrain folder (使用绝对路径)
MODEL_FOLDER = os.path.join(PROJECT_ROOT, "pretrain")

# path to temp folder (使用绝对路径)
TEMP_PATH = os.path.join(PROJECT_ROOT, "tmpdir")

# path to unoffical model config (使用绝对路径)
...
UNOFFICIAL_MODEL = os.path.join(PROJECT_ROOT, "config_unofficial")

# path to vr model params (使用绝对路径)
# path to unofficial vr model params: UNOFFICIAL_MODEL/vr_modelparams
VR_MODELPARAMS = os.path.join(PROJECT_ROOT, "configs", "vr_modelparams")

# url for check for updates
UPDATE_URL = "https://github.com/SUC-DriverOld/MSST-WebUI/releases/latest"

# pretrained SOME weight (使用绝对路径)
SOME_WEIGHT = os.path.join(PROJECT_ROOT, "tools", "SOME_weights", "model_steps_64000_simplified.ckpt")
SOME_CONFIG = os.path.join(PROJECT_ROOT, "configs_backup", "config_some.yaml")

# path to the theme folder (使用绝对路径)
THEME_FOLDER = os.path.join(PROJECT_ROOT, "tools", "themes")
